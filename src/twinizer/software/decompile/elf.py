...
class ELFDecompiler:
    ...
    def __init__(self, use_ghidra: bool = True, use_ida: bool = False, 
                 use_retdec: bool = False, output_dir: Optional[str] = None):
        """
    ...
    def _check_decompilers(self):
        ...
        if self.use_ghidra:
            ...
            if not ghidra_path or not os.path.exists(ghidra_path):
                console.print("[yellow]Warning: Ghidra not found. Set GHIDRA_HOME environment variable.[/yellow]")
        ...
        if self.use_ida:
            ...
            if not ida_path or not os.path.exists(ida_path):
                console.print("[yellow]Warning: IDA Pro not found. Set IDA_HOME environment variable.[/yellow]")
                self.use_ida = False
        
        if self.use_retdec:
            # Check for RetDec
            try:
                subprocess.run(["retdec-decompiler", "--version"], 
                               stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
            except FileNotFoundError:
                console.print("[yellow]Warning: RetDec not found. Make sure it's installed and in your PATH.[/yellow]")
                self.use_retdec = False
        
        if not any([self.use_ghidra, self.use_ida, self.use_retdec]):
            console.print("[yellow]Warning: No decompilers available. Results will be limited.[/yellow]")
    
    def decompile(self, elf_path: str, output_format: str = "c", 
                  function_name: Optional[str] = None) -> Dict[str, str]:
        """
        ...
        with Progress(
            ...
            if self.use_ghidra:
                task = progress.add_task("[green]Decompiling with Ghidra...", total=None)
                output_path = self._decompile_with_ghidra(elf_path, output_format, function_name)
            ...
            if self.use_ida:
                task = progress.add_task("[green]Decompiling with IDA Pro...", total=None)
                output_path = self._decompile_with_ida(elf_path, output_format, function_name)
            ...
            if self.use_retdec:
                task = progress.add_task("[green]Decompiling with RetDec...", total=None)
                output_path = self._decompile_with_retdec(elf_path, output_format, function_name)
    ...
    def _decompile_with_ghidra(self, elf_path: str, output_format: str, 
        ...
        script_content = self._generate_ghidra_script(output_path, output_format, function_name)
    ...
    def _generate_ghidra_script(self, output_path: str, output_format: str, 
                               function_name: Optional[str] = None) -> str:
        """
...
"""
    ...
    def _decompile_with_ida(self, elf_path: str, output_format: str, 
                           function_name: Optional[str] = None) -> str:
        """
    ...
    def _decompile_with_retdec(self, elf_path: str, output_format: str, 
                              function_name: Optional[str] = None) -> str:
        """
...
def decompile_elf(elf_path: str, output_dir: Optional[str] = None, 
                 output_format: str = "c", function_name: Optional[str] = None,
                 use_ghidra: bool = True, use_ida: bool = False, 
                 use_retdec: bool = False) -> Dict[str, str]:
    """
