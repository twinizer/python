...
@kicad_deps_group.command(name="sch-to-mermaid", help="Convert KiCad schematic to Mermaid diagram with dependencies")
...
@click.option("--include-dependencies/--no-include-dependencies", default=True,
...
@kicad_deps_group.command(name="sch-to-bom", help="Generate BOM from KiCad schematic with dependencies")
@click.argument("schematic_file", type=click.Path(exists=True))
@click.option("--format", type=click.Choice(["csv", "json", "html", "xlsx"]), default="csv",
              help="Output format")
@click.option("--output", type=click.Path(), default=None,
