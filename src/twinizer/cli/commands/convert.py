...
@click.option("--ocr/--no-ocr", default=False,
              help="Enable OCR for text in images")
...
@click.option("--source-dir", "-d", default=None,
              help="Source directory (defaults to ./source)")
...
def schematic_to_image(file, output, format, dpi, source_dir):
    ...
    if file.endswith(('.sch', '.kicad_sch', '.kicad_pcb')):
        ...
        console.print(f"[bold yellow]Warning:[/bold yellow] Unsupported schematic file format: {file}")
...
@click.option("--language", "-l", default=None,
              help="Target language (c, python, etc.)")
@click.option("--source-dir", "-d", default=None,
              help="Source directory (defaults to ./source)")
def binary_to_source(file, output, language, source_dir):
    ...
    console.print(f"[bold blue]Converting binary to {language} source code:[/bold blue] {file}")
    ...
    if file.endswith('.pyc'):
        ...
        try:
            ...
            console.print(f"[bold green]Decompilation successful:[/bold green] {output_path}")
        except Exception as e:
        ...
        try:
            ...
            console.print(f"[bold green]Bindings generation successful:[/bold green] {output_path}")
        except Exception as e:
        ...
        console.print(f"[bold yellow]Warning:[/bold yellow] Unsupported binary file format: {file}")
...
def doc_to_tree(file, output, source_dir):
    ...
    console.print(f"[bold blue]Converting documentation to tree structure:[/bold blue] {file}")
    ...
    if file.endswith(('.md', '.markdown')):
        ...
        console.print(f"[bold yellow]Warning:[/bold yellow] Unsupported documentation file format: {file}")
        return
...
def schematic_to_netlist(file, output, source_dir):
    ...
    if file.endswith(('.sch', '.kicad_sch')):
        from twinizer.hardware.kicad.converters import convert_kicad_schematic_to_netlist
        try:
...
def pcb_to_gerber(file, output_dir, source_dir):
    ...
    if file.endswith('.kicad_pcb'):
        ...
        console.print(f"[bold yellow]Warning:[/bold yellow] Unsupported PCB file format: {file}")