...
class CPPLinter:
    ...
    def analyze(self, 
        ...
        if self.use_cppcheck:
            results["cppcheck"] = self.run_cppcheck(target_path, output_format=output_format)
        ...
        if self.use_clang_tidy:
            ...
            if os.path.isdir(target_path):
                ...
                for root, _, files in os.walk(target_path):
                    for file in files:
                        if file.endswith(('.c', '.cpp', '.cc', '.cxx', '.h', '.hpp', '.hxx')):
                            file_path = os.path.join(root, file)
                            result = self.run_clang_tidy(file_path, output_format=output_format)
                ...
                results["clang_tidy"] = self.run_clang_tidy(target_path, output_format=output_format)
        
        if self.use_cpplint:
            results["cpplint"] = self.run_cpplint(target_path, output_format=output_format)
        ...
        if output_format == "json":
                ...
                if tool == "cppcheck":
                    ...
                    if isinstance(tool_results.get("result"), list):
                        for file_result in tool_results["result"]:
                            ...
                            if isinstance(file_issues, dict) and "Diagnostics" in file_issues:
                                md_lines.append("| Line | Column | Severity | Message |\n")
                                md_lines.append("|------|--------|----------|--------|\n")
            ...
            for tool, tool_results in results.items():
                ...
                if tool == "cppcheck" and isinstance(tool_results.get("result"), list):
                    ...
                    html_lines.append("        <tr><th>File</th><th>Line</th><th>Severity</th><th>Message</th></tr>")
                    ...
                    html_lines.append("    </table>")
                elif tool == "clang_tidy" and isinstance(tool_results.get("result"), list):
                    for file_result in tool_results["result"]:
                        ...
                        if isinstance(file_issues, dict) and "Diagnostics" in file_issues:
                            html_lines.append("    <table>")
                            html_lines.append("        <tr><th>Line</th><th>Column</th><th>Severity</th><th>Message</th></tr>")
                    ...
                    html_lines.append("    <table>")
                    html_lines.append("        <tr><th>File</th><th>Line</th><th>Column</th><th>Message</th></tr>")
...
def analyze_cpp_code(target_path: str, 
